...
class Objective(object):
    ...
    def calc_joint_contour(
        ...
        if contour_df is None:
            ...
            try:
                destpid = np.where(self.sp_graph.perm_idx[:self.sp_graph.n_observed_nodes]==destid)[0][0]
            except:
            ...
            if usew is None:
                self.sp_graph.fit(lamb=lamb, optimize_q=optimize_q, lamb_q=lamb_q, option='default', verbose=False);
                self.inv(); self.grad(reg=False)
            ...
            checkpoints = {int(np.percentile(range(len(self.sp_graph.perm_idx)),25)): 25, int(np.percentile(range(len(self.sp_graph.perm_idx)),50)): 50, int(np.percentile(range(len(self.sp_graph.perm_idx)),75)): 75}
            ...
            for ie, e in enumerate(randedge):
                ...
                try:
                    self.sp_graph.fit(lamb=lamb, optimize_q=optimize_q, lamb_q=lamb_q, long_range_edges=[e], option='onlyc', verbose=False)
            ...
            for i, row in joint_contour_df.iterrows():
                ...
                try:
                    self.sp_graph.fit(lamb=lamb, optimize_q=optimize_q, lamb_q=lamb_q, long_range_edges=[row['(source, dest.)']], option='onlyc', verbose=True)
    ...
    def calc_contour(
        ...
        try:
            destpid = np.where(self.sp_graph.perm_idx[:self.sp_graph.n_observed_nodes]==destid)[0][0] #-> 0:(o-1)
        except:
            print('invalid ID for destination deme, please specify valid sampled ID from graph or from output of extract_outliers function\n')
            return None
        ...
        for ie, e in enumerate(randedge):
            ...
            # e2 = (np.where(self.sp_graph.perm_idx==e[0])[0][0], destpid) # -> contains the permuted ids, so 0:(o-1) is sampled (useful for indexing Linv & Lpinv)
    ...
    def extract_outliers(self, fdr=0.25, res_dist=None, verbose=False):
        ...
        for k in np.where(bh)[0]:
            ...
            # Gi = self.sp_graph.genotypes[self.sp_graph.nodes[self.sp_graph.perm_idx[x[-1]]]['sample_idx'], :]
            Gi = self.sp_graph.genotypes[self.sp_graph.nodes[self.sp_graph.perm_idx[x[-1]]]['sample_idx'], :].astype('int').T
            Ga1 = np.zeros((Gi.shape[0], Gi.shape[1], 2), dtype=int)
            Ga1[Gi == 1, 1] = 1
            Ga1[Gi == 2] = 1

            Gi = self.sp_graph.genotypes[self.sp_graph.nodes[self.sp_graph.perm_idx[y[-1]]]['sample_idx'], :].astype('int').T
            ...
            ls.append([self.sp_graph.perm_idx[x[-1]], self.sp_graph.perm_idx[y[-1]], tuple(self.sp_graph.nodes[self.sp_graph.perm_idx[x[-1]]]['pos'][::-1]), tuple(self.sp_graph.nodes[self.sp_graph.perm_idx[y[-1]]]['pos'][::-1]), pvals[k], emp_dist[k]-fit_dist[k], fst])
        ...
        for k in range(len(ls)):
            ...
            if resc.x<1e-3 and rescopp.x<1e-3:
                ...
                if np.abs(rescopp.fun - resc.fun) <= 5:
                    newls.append([self.sp_graph.perm_idx[y[k]], self.sp_graph.perm_idx[x[k]], tuple(self.sp_graph.nodes[self.sp_graph.perm_idx[y[k]]]['pos'][::-1]), tuple(self.sp_graph.nodes[self.sp_graph.perm_idx[x[k]]]['pos'][::-1]), pvals[k], emp_dist[k]-fit_dist[k], ls[k][-1]])
                else:
                    # if the "opposite" direction has a much higher log-likelihood then replace it entirely 
                    if rescopp.fun < resc.fun:
                        ls[k][0] = self.sp_graph.perm_idx[y[k]]
                        ls[k][1] = self.sp_graph.perm_idx[x[k]]
