    ...
    for fold in range(n_folds):
        ...
        for a, alpha in enumerate(alpha_grid):
            ...
            for i, lamb in enumerate(lamb_grid):
                print('\riteration lambda={}/{} alpha={}/{}'.format(i+1,n_lamb,a+1,n_alpha), end='')
                # fit on train set
                lamb = float(lamb)
                alpha = float(alpha)
                sp_graph_train.fit(lamb=lamb, 
                                   w_init=w_init, 
                                   s2_init=s2_init, 
                                   alpha=alpha, 
                                   factr=factr, 
...
def setup_kfold_cv(sp_graph, n_splits=5, random_state=12):
    ...
    Returns:
        is_train (:obj:`numpy.ndarray`): n x k matrix storing boolean values determining
...
def copy_spatial_graph(sp_graph, subsample_idx):
    ...
    sp_graph_copy._assign_samples_to_nodes(sp_graph_copy.sample_pos, sp_graph_copy.node_pos)
    ...
    sp_graph_copy.n_samples_per_obs_node_permuted = n_samps[: sp_graph_copy.n_observed_nodes]
    ...
    if sp_graph.scale_snps:
        sp_graph_copy.frequencies = sp_graph_copy.frequencies / np.sqrt(sp_graph.mu * (1 - sp_graph.mu))

    # compute precision
    sp_graph_copy.comp_precision(s2=1)

    # estimate sample covariance matrix
    sp_graph_copy.S = sp_graph_copy.frequencies @ sp_graph_copy.frequencies.T / sp_graph_copy.n_snps    
...
def predict_SNPs(sp_graph, sp_graph_train, sp_graph_test):
    ...
    pred_frequencies = mu_frequencies + cov_te_tr @ np.linalg.solve(cov_tr_tr, frequencies[train_ids, :] - mu_frequencies)
    l2_err = np.linalg.norm(pred_frequencies*mu_f - frequencies[test_ids]*mu_f)**2/(len(test_ids) * n_snps)    
